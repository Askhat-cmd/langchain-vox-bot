...
|---|---|
| `app/voximplant/scenario_barge-in_2.js` | **Voximplant-сценарий** обработки звонка, речи и **умного barge-in** |
| `app/backend/main.py` | FastAPI-сервер: WebSocket, API, **авторизация** и статика `/logs-ui/` |
| `app/backend/rag/agent.py` | LangChain-агент: RAG, маршрутизация general/tech |
| `scripts/create_embeddings.py` | Скрипт для создания/пересоздания базы векторов (атомарно) |
| `scripts/test_agent.py` | **Скрипт для автоматического тестирования** качества ответов агента |
| `app/frontend/logs-ui/` | Файлы фронтенда для админ-панели (React/JS) |
...
Агент (модель) по инструкции в промптах завершает каждое законченное предложение символом `|`. Бэкенд текст не изменяет — он транзитит ответы по WebSocket, а сценарий `scenario_barge-in_2.js` учитывает эти разделители:
...
Подсказка: если речь звучит «слипно», проверьте, что в `prompts.json` (поле `qa_system_prompt`) есть правило про завершение каждого предложения символом `|`.

...
6. `scenario_barge-in_2.js` на Voximplant воспроизводит текст сразу (TTS), используя сегментацию по `|`, очередь TTS и **умный barge-in** с защитой от эха.
7. После завершения звонка, `main.py` сохраняет полный лог в `SQLite` базу (`data/log_db.sqlite`).

---

## 🎙️ Voximplant: сегментация по `|`, очередь TTS и barge-in

На бэкенде конец каждого предложения помечается символом `|`. Сценарий `scenario_barge-in_2.js` учитывает это:

- **Сегментация**: входящий stream аккумулируется до символа `|`. Как только он найден, фраза немедленно отправляется в озвучку (TTS) без дополнительной задержки. Хвост без `|` озвучивается через страховочный таймер `SPEECH_END_TIMEOUT`.
- **Очередь TTS**: предложения ставятся в очередь и проигрываются по одному; это даёт ровную просодию и отсутствие «рваной» речи.
- **Barge-in**: при событиях ASR `CaptureStarted`/`InterimResult` бот прерывает TTS. Введена защита от эха `BARGE_IN_GUARD_MS` — первые миллисекунды после старта TTS баргин игнорируется.
- **Мгновенный флаш реплики**: если доступно событие `ASREvents.CaptureStopped`, пользовательская реплика отправляется на бэкенд сразу по окончанию захвата речи. Если событие недоступно у провайдера, используется таймер `INPUT_DEBOUNCE_MS`.

Текущие значения в сценарии:
...
Совместимость: подписки на ASR‑события выполняются безопасно; если провайдер не поддерживает, сценарий автоматически использует доступные события.
